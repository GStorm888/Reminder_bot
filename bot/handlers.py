...
def register_handlers(bot: TeleBot):
    ...
    def register_user(message):
        ...
        if (
            ...
            bot.send_message(
                message.chat.id, "пользователь с таким именем уже есть, введи другое"
            )
        ...
        bot.send_message(
            message.chat.id, "готово, показать команды?", reply_markup=markup
        )
    ...
    def add_reminder(message):
        ...
        bot.send_message(
            message.chat.id,
            "Выберите дни в которые нужны напоминания",
            reply_markup=markup,
        )
    ...
    def delete_reminder(message):
        ...
        monday = types.InlineKeyboardButton(
            text="📅🟦Понедельник", callback_data="delete_monday"
        )
        tuesday = types.InlineKeyboardButton(
            text="📅🟧Вторник", callback_data="delete_tuesday"
        )

        wednesday = types.InlineKeyboardButton(
            text="📅🟩Среда", callback_data="delete_wednesday"
        )
        thursday = types.InlineKeyboardButton(
            text="📅🟥Четверг", callback_data="delete_thursday"
        )

        friday = types.InlineKeyboardButton(
            text="📅🟪Пятница", callback_data="delete_friday"
        )
        saturday = types.InlineKeyboardButton(
            text="📅🟫Суббота", callback_data="delete_saturday"
        )

        sunday = types.InlineKeyboardButton(
            text="📅⬛Воскресенье", callback_data="delete_sunday"
        )
    ...
    def processing_day_reminder_delete(message):
        ...
        if reminders is None:
            bot.send_message(
                message.chat.id, f"нет напоминаний на выбраный день {day_name}"
            )
            return
        ...
        if all_reminders is not None:
            for reminder in all_reminders:
                if (
                    ...
                    bot.send_message(
                        user.telegram_id, f"⏰Напоминание!{reminder.text_reminder}"
                    )
        time.sleep(60)
